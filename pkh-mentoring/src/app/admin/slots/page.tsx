  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 bg-navy hover:bg-navy-light text-white font-medium py-2 px-4 rounded-md transition-colors text-sm">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-sand-dark bg-sand">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium text-warm-grey uppercase tracking-wider">Zoom Link</th>
            ...
            <tbody>
              {slots.length === 0 && (
                <tr><td colSpan={8} className="px-6 py-8 text-center text-warm-grey">No slots yet. Click &quot;Add Slot&quot; to create one.</td></tr>
              )}
              {slots.map((slot, i) => {
                const w1 = slot.instances.find((inst) => inst.weekNumber === 1);
                const w2 = slot.instances.find((inst) => inst.weekNumber === 2);
                return (
                  <tr key={slot.id} className={i % 2 === 1 ? "bg-sand" : "bg-white"}>
                    ...
                    <td className="px-6 py-3 text-sm text-charcoal">
                      {slot.zoomLink ? (
                        <div className="flex items-center gap-2">
                          <span className="truncate max-w-[120px]">{slot.zoomLink}</span>
                          <button onClick={() => copyZoomLink(slot.zoomLink!, slot.id)} className="text-warm-grey hover:text-navy flex-shrink-0" title="Copy zoom link">
                            {copiedZoom === slot.id ? <Check size={14} className="text-success" /> : <Copy size={14} />}
                          </button>
                        </div>
                      ...
                      <div className="w-24">
                        <div className="h-2 bg-sand-dark/50 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${slot.totalCapacity > 0 ? Math.min((slot.totalStudents / slot.totalCapacity) * 100, 100) : 0}%` }}
                          />
                        </div>
                        <div className="text-xs text-warm-grey mt-1">{slot.totalStudents}/{slot.totalCapacity}</div>
                      </div>
                    </td>
                    <td className="px-6 py-3">
                      <button onClick={() => setEditSlot(slot)} className="text-sm text-navy hover:text-navy-light font-medium">Edit</button>
                    </td>
                  </tr>
                );
              })}
      ...
      {showAddModal && <AddSlotModal onClose={() => setShowAddModal(false)} onCreated={() => { setShowAddModal(false); fetchSlots(); }} />}
      {editSlot && <EditSlotModal slot={editSlot} onClose={() => setEditSlot(null)} onUpdated={() => { setEditSlot(null); fetchSlots(); }} />}
    </div>
  );
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/slots", {
        ...
        body: JSON.stringify({ dayOfWeek: parseInt(dayOfWeek, 10), time, zoomLink }),
      });
      if (res.ok) { onCreated(); } else { const data = await res.json(); setError(data.error || "Failed to create slot"); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-sand-dark">
          ...
          <button onClick={onClose} className="text-warm-grey hover:text-navy"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-charcoal mb-1">Day</label>
            <select value={dayOfWeek} onChange={(e) => setDayOfWeek(e.target.value)} className="w-full px-3 py-2 border border-sand-dark rounded-md focus:outline-none focus:ring-2 focus:ring-gold bg-white">
              {DAYS.map((d) => <option key={d.value} value={d.value}>{d.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-charcoal mb-1">Time</label>
            <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full px-3 py-2 border border-sand-dark rounded-md focus:outline-none focus:ring-2 focus:ring-gold" />
          </div>
          <div>
            <label className="block text-sm font-medium text-charcoal mb-1">Zoom Link</label>
            <input type="url" value={zoomLink} onChange={(e) => setZoomLink(e.target.value)} placeholder="https://zoom.us/j/..." className="w-full px-3 py-2 border border-sand-dark rounded-md focus:outline-none focus:ring-2 focus:ring-gold" />
          </div>
          {error && <div className="p-3 bg-error/10 border border-error/20 rounded-md text-error text-sm">{error}</div>}
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-charcoal hover:bg-sand rounded-md transition-colors">Cancel</button>
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm font-medium bg-navy hover:bg-navy-light text-white rounded-md transition-colors disabled:opacity-50">{saving ? "Creating..." : "Create Slot"}</button>
          </div>
        </form>
      </div>
    </div>
...
function EditSlotModal({ slot, onClose, onUpdated }: { slot: Slot; onClose: () => void; onUpdated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-sand-dark">
          <h2 className="text-lg font-semibold text-navy">Edit: {slot.displayName}</h2>
          <button onClick={onClose} className="text-warm-grey hover:text-navy"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-charcoal hover:bg-sand rounded-md transition-colors">Cancel</button>
            <button type="submit" disabled={saving} className="px-4 py-2 text-sm font-medium bg-navy hover:bg-navy-light text-white rounded-md transition-colors disabled:opacity-50">{saving ? "Saving..." : "Save Changes"}</button>
          </div>
        </form>
      </div>
    </div>
